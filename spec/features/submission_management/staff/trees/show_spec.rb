...
RSpec.describe 'Managing a submission tree as a staff member' do
  ...
  let(:student_term_registration) { FactoryBot.create(:term_registration, :student, term: term, tutorial_group: term_registration.tutorial_group) }

  let(:submission) { FactoryBot.create(:submission, exercise: exercise) }
  let(:exercise) { FactoryBot.create(:exercise, term: term) }
  let!(:exercise_registration) { FactoryBot.create(:exercise_registration, term_registration: student_term_registration, exercise: exercise, submission: submission)}
