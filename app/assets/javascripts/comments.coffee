$(document).on 'click', "[id^=explanations_comment_save]", () ->
  ...
  $("##{evaluation_comment_button($(this))}").html('Explanations' + comment_count_badge(count + 1))
...
$(document).on 'click', "[id^=feedback_comment_save]", () ->
  ...
  count = comment_count($(this))
  $("[id^=feedback_button]").html('Feedback' + comment_count_badge(count + 1))
...
$(document).on 'click', "[id^=internal_notes_comment_save]", () ->
  ...
  count = comment_count($(this))
  $("[id^=internal_notes_button]").html('Internal Notes' + comment_count_badge(count + 1))
