module ZipGeneration
  ...
  def generate_zip_and_assign_to_file!(directory)
    ...
    zip_filename = File.join(zips_path, "#{term.course.title.parameterize}-#{term.title.parameterize}-#{Time.now.strftime '%Y%m%d-%H%M'}-#{id}.zip")
    ...
    cmd = %(cd "#{directory}" && zip -r "#{zip_filename}" *)
    `#{cmd}`
