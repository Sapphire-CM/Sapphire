class SubmissionAssetsController < ApplicationController
  def show
    ...
    logger.info @submission_asset.file.current_path
    send_file @submission_asset.file.current_path, filename: @submission_asset.file.identifier, type: @submission_asset.content_type, disposition: :inline
  end
  def new
  ...
  def create
    ...
    @submission_asset.content_type = params[:submission_asset][:file].content_type
  ...
  def submission_assets_params
    params.require(:submission_asset).permit(:file, :submission_id)
  end
end
