class SubmissionEvaluationsController < TermResourceController
  def show
    ...
    redirect_to new_term_submission_evaluation_path(current_term, current_submission) unless @submission_evaluation.present?
  end
  ...
  def new
    ...
    if @submission_evaluation.present?
      redirect_to edit_term_submission_evaluation_path(current_term, current_submission)
      return
    end
  ...
  def create
    @submission_evaluation = current_submission.submission_evaluation
    if @submission_evaluation.present?
      redirect_to term_submission_evaluation_path(current_term, current_submission), :alert => "Error - Evaluation already present"
      return
    end
    ...
    if @submission_evaluation.save
      ...
      redirect_to term_submission_evaluation_path(current_term, current_submission), :notice => "Evaluation has been created"
    else
      render :new
    end
  end
  ...
  def update
    ...
    unless @submission_evaluation.present?
      redirect_to new_term_submission_evaluation_path(current_term, current_submission)
      return
    end
    ...
    if @submission_evaluation.update_attributes(params[:submission_evaluation])
      @submission_evaluation.update_evaluation_result!
      redirect_to term_submission_evaluation_path(current_term, current_submission), :notice => "Evaluation has been updated"
    else
