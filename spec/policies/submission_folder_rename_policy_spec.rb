...
RSpec.describe SubmissionFolderRenamePolicy, type: :policy do

  let(:submission) { FactoryBot.create(:submission, exercise: exercise) }
  let(:directory) { submission.tree.resolve( "directory") }
  let(:submission_folder_rename) { SubmissionFolderRename.new(submission: submission, directory: directory) }
  subject { described_class.new(account, submission_folder_rename) }
