  ...
  def student_groups_for_table
    @student_groups ||= begin
      ...
      student_groups = if @exercise.group_submission?
        case @order
        when :group_name
          student_groups.order(:title)
        when :group_name_inverse
          student_groups.order(:title).reverse_order
        else
          @order = :group_name
          student_groups.order(:title)
        end
        ...
        case @order
        when :surname_forename
          student_groups.load.sort_by{ |sg| sg.students.first.reverse_fullname }
        when :matriculation_number
          student_groups.load.sort_by{ |sg| sg.students.first.matriculation_number }
        when :email
          student_groups.load.sort_by{ |sg| sg.students.first.email }
        when :surname_forename_inverse
          student_groups.load.sort_by{ |sg| sg.students.first.reverse_fullname }.reverse
        when :matriculation_number_inverse
          student_groups.load.sort_by{ |sg| sg.students.first.matriculation_number }.reverse
        when :email_inverse
          student_groups.load.sort_by{ |sg| sg.students.first.email }.reverse
        else
          @order = :surname_forename
          student_groups.load.sort_by{ |sg| sg.students.first.reverse_fullname }
        end
      end
